...
std::vector<mp_bbox_t> MediapipeImpl::Process(uint8_t* data, int width, int height) 
{
    if (data == nullptr){
    ...
    const std::vector<Detection>& detections = packet.Get<std::vector<Detection>>(); 

    std::vector<mp_bbox_t> detection_data(detections.size());

    for (size_t i = 0; i < detection_data.size(); ++i) {
        const auto& bbox = detections[i].location_data().relative_bounding_box();
        detection_data[i] = {bbox.xmin(), bbox.ymin(), bbox.width(), bbox.height(), detections[i].score(0), detections[i].label(0), int(detections[i].detection_id())};
    }
